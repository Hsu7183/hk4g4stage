...
function run(raw, meta) {
  hideErr();
  const parsed = analyse(raw, meta);
  if (!parsed) return;
  const { tsArr, seq, trades, kpi, shortName, paramsText } = parsed;
...
/* ===== 解析：第一行若不是交易就當「參數列」 ===== */
function analyse(raw, meta={filename:''}) {
  const lines = raw.trim().split(/\r?\n/).filter(Boolean);
  if (!lines.length) { showErr('空檔案。'); return null; }

  let paramLine = '';
  if (!isTradeLine(lines[0])) { paramLine = lines.shift(); }
  ...
  const paramsText = paramLine || nameParams;
