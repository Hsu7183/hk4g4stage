...
function analyse(raw) {
  ...
  const rows = raw
    .split(/\r?\n/)
    .map(s => s.replace(/\uFEFF/g,'').trim()) // 去 BOM/不可見字元
    .filter(Boolean);

  if (!rows.length) { alert('空檔案'); return; }

  // 若第一行是「全是數字/小數」的參數列 => 直接略過
  ...
  let startIdx = 0;
  if (isParamLine(rows[0])) startIdx = 1;
  ...
  for (let i = startIdx; i < rows.length; i++) {
    ...
    // 取「最後一個欄位」當動作，避免中間還有其他欄位
    let act = parts[parts.length - 1].replace(/\s/g,'');
    if (!ACTS.has(act)) continue; // 不認得就略過

    // 取第一欄時間、第二欄價格
    let tsRaw = parts[0].replace(/\D/g,''); // 只留數字
