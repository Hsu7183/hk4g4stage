...
filesInput.addEventListener('change', async (e) => {
  ...
  for (const [idx, f] of files.entries()) {
    try {
      ...
      const { shortName, paramsText } = parseFilename(f.name); // 參數取自檔名
...
function buildKPI(tr, seq) {
  ...
  return { 全部:make(tr,seq), 多單:make(tr.filter(t=>t.pos?.side==='L'), {tot:seq?.lon}), 空單:make(tr.filter(t=>t.pos?.side==='S'), {tot:seq?.sho}) };
}
function emptyStats(){
