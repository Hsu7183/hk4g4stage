...
function parseTS(a,b){var d1=onlyDigits(a),d2=onlyDigits(b);var cand=d1;if(d1.length<=8&&d2)cand=d1+d2; if(cand.length>=14)return cand.slice(0,14); if(cand.length>=12)return cand.slice(0,12); if(cand.length>=8)return cand.slice(0,8)+'0000'; return '';}
function parseLine(line){
  if(!line) return null;
  var s=line.replace(/[，,]/g,' ').replace(/\t+/g,' ').replace(/\s+/g,' ').trim();
  var parts=s.split(' '); if(parts.length<3) return null;
  var ts=parseTS(parts[0],parts[1]); var priceIdx=ts?2:1;
  ...
  var act=''; for(var j=pIndex+1;j<parts.length;j++){ if(!/^[-+]?\d+(?:\.\d+)?$/.test(parts[j])){act=normAct(parts[j]);break;} }
  if(!act) return null;
...
function paramsDisplay(s){if(!s)return '—';var t=s.replace(/[，,]/g,' ').trim().split(/\s+/).filter(Boolean);var ok=t.length>0&&t.every(function(x){return /^[-+]?\d+(?:\.\d+)?$/.test(x);});return ok?t.map(function(x){return String(Math.trunc(parseFloat(x)));}).join(' / '):s;}
...
filesInput.addEventListener('change',function(e){
  ...
  var failed=0,first=false;
  (function loop(i){
    if(i>=files.length) return;
    var f=files[i];
    readFile(f).then(function(raw){
      var need=!first;
      var res=analyse(raw,{needFull:need,filename:f.name});
      rowsData.push({filename:f.name,shortName:res.shortName,paramsText:res.paramsText,fileRef:f,kpi:res.kpi,sortCache:sortCache(res.kpi),equitySeq:need?res.eq:null,tsSeq:need?res.ts:null,trades:need?res.trades:null});
      appendRow(res.shortName,res.paramsText,res.kpi);
      if(need&&res.ts&&res.ts.length){ drawChart(res.ts,res.eq.tot,res.eq.lon,res.eq.sho,res.eq.sli); renderTrades(res.trades); renderKPI(res.kpi); showParam(res.shortName,res.paramsText); first=true; }
    }).catch(function(err){console.error(err);failed++;})
    .finally(function(){updateStat(i+1,files.length,failed);loop(i+1);});
  })(0);
});
btnClear.onclick=function(){ filesInput.value=''; thead.innerHTML=''; tbody.innerHTML=''; rowsData=[]; updateStat(0,0,0); if(chart) chart.destroy(); tradesBody.innerHTML='<tr><td colspan="13" style="color:#777">尚未載入</td></tr>'; kpiBlocks.innerHTML=''; paramBar.hidden=true; };

function analyse(raw,opt){
  opt=opt||{};
  var rows=(raw||'').replace(/^\uFEFF/,'').trim().split(/\r?\n/).filter(Boolean);
  if(!rows.length) throw new Error('空檔案');
  var paramLine=''; if(!parseLine(rows[0])) paramLine=rows.shift();
  ...
  var pf=parseName(opt.filename||''); var params=paramsDisplay(paramLine||pf.params);
...
function buildHeader(){
  ...
  for(var g=0;g<GROUPS.length;g++){
    for(var k=0;k<KPI_ORDER.length;k++){
      cells.push('<th class="sortable nowrap" data-key="'+GROUPS[g]+'.'+KPI_ORDER[k][1]+'">'+GROUPS[g]+'-'+KPI_ORDER[k][0]+'</th>');
    }
  }
  ...
  [].forEach.call(ths,function(th){
    th.addEventListener('click',function(){
      ...
      curDir=(curKey===key?(curDir==='asc'?'desc':'asc'):'asc'); curKey=key;
...
function appendRow(short,params,kpi){
  var tds=['<td class="nowrap" title="'+escapeHTML(short)+'">'+escapeHTML(short)+'</td>',
           '<td class="nowrap" title="'+escapeHTML(params)+'">'+escapeHTML(params)+'</td>'];
...
function redrawTop(){
  var first=null; for(var i=0;i<rowsData.length;i++){ if(rowsData[i].kpi){first=rowsData[i];break;} }
  ...
  function go(){ drawChart(first.tsSeq,first.equitySeq.tot,first.equitySeq.lon,first.equitySeq.sho,first.equitySeq.sli); renderTrades(first.trades); renderKPI(first.kpi); showParam(first.shortName,first.paramsText); }
  if(!first.tsSeq){ readFile(first.fileRef).then(function(raw){ var r=analyse(raw,{needFull:true,filename:first.filename}); first.equitySeq=r.eq; first.tsSeq=r.ts; first.trades=r.trades; go(); }); } else go();
}
...
function updateStat(done,total,failed){ if(!total){loadStat.textContent='';return;} loadStat.textContent='載入：'+done+'/'+total+'，成功：'+(done-failed)+'，失敗：'+failed; }
