...
filesInput.addEventListener('change',function(e){
  ...
  (function loop(i){
    ...
    readFile(f).then(function(raw){
      var res=analyse(raw,{needFull:!first,filename:f.name});
      rowsData.push({filename:f.name,shortName:res.shortName,paramsText:res.paramsText,fileRef:f,kpi:res.kpi,sortCache:sortCache(res.kpi),equitySeq:res.eq,tsSeq:res.ts,trades:res.trades});
      appendRow(res.shortName,res.paramsText,res.kpi);
      if(!first && res.ts && res.ts.length){
        drawChart(res.ts,res.eq.tot,res.eq.lon,res.eq.sho,res.eq.sli);
        renderTrades(res.trades); renderKPI(res.kpi);
        showParam(res.shortName,res.paramsText); first=true;
      }
...
function sortCache(kpi){
  var flat={}; GROUPS.forEach(function(g){var obj=kpi[g]||{}; KPI_ORDER.forEach(function(p){var v=obj[p[1]]; if(typeof v==='number')flat[g+'.'+p[1]]=v; else if(typeof v==='string'){ if(v.slice(-1)==='%')flat[g+'.'+p[1]]=parseFloat(v); else if(v==='—')flat[g+'.'+p[1]]=-Infinity; else if(v==='∞')flat[g+'.'+p[1]]=Number.POSITIVE_INFINITY; else flat[g+'.'+p[1]]=parseFloat(v.replace(/,/g,''))||-Infinity; } else flat[g+'.'+p[1]]=-Infinity;});});
  return flat;
}
