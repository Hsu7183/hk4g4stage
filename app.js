...
function analyse(raw){
  ...
  renderTable(tr);
  drawChart(ymSeq,tot,lon,sho,sli);   // *** 不再壓縮，直接每筆畫 ***
}
...
function drawChart(ymArr,T,L,S,P){
  ...
  chart=new Chart(cvs,{
    ...
    data:{
      labels:X,
      datasets:[
        mkLine(T,'#fdd835'), mkLine(L,'#e53935'),
        mkLine(S,'#43a047'), mkLine(P,'#000000'),
        mkLast(T,'#fdd835'), mkLast(L,'#e53935'),
        mkLast(S,'#43a047'), mkLast(P,'#000000'),
        mkMark(T,maxI,'#e53935'), mkMark(T,minI,'#43a047')
      ]
    },
    options:{
