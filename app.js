...
function compressAndDraw(ts,tot,L,S,P){
  const end=new Map(); ts.forEach((t,i)=>end.set(ym(t),i));
  const idx=[...end.values()].sort((a,b)=>a-b);
  drawChart(
    idx.map(i=>ym(ts[i])),
    idx.map(i=>tot[i]),
    idx.map(i=>L[i]),
...
function drawChart(lbl,T,L,S,P){
  ...
  /* --- 基本線型（階梯＋每點實心圓） --- */
  const stepLine=(col,w)=>({
    borderColor:col,borderWidth:w,stepped:true,
    pointRadius:3,pointBackgroundColor:col,pointBorderColor:col,
    fill:false
  });
  const lastPoint=(arr,col)=>({
    data:arr.map((v,i)=>i===N?v:null),
      ...
      plugins:{
        ...
        datalabels:{
          ...
          display:ctx=>{
            ...
            return lab===''||lab==='總'||lab==='多'||lab==='空'||lab==='滑'?false:true;
          },
