...
function analyse(raw){
  ...
  const q=[],tr=[],dates=[],tot=[],lon=[],sho=[],sli=[];
...
function drawChart(dateArr,T,L,S,P){
  ...
  const X=dateArr.map(ts=>{
    const ym=ts.slice(0,6),d=+ts.slice(6,8),days=new Date(+ym.slice(0,4),+ym.slice(4,6),0).getDate();
    return monthIdx[ym]+((d-0.5)/days)-0.5;       // -0.5 使月中為 0
  });
  ...
  const mkLast=(d,col)=>({data:d.map((v,i)=>i===d.length-1?v:null),showLine:false,pointRadius:6,
      ...
      datalabels:{display:true,anchor:'center',align:'left',offset:8,formatter:v=>v?.toLocaleString('zh-TW')??'',clip:false,color:'#000',font:{size:10}}});
  const mkMark=(d,i,col)=>({data:d.map((v,j)=>j===i?v:null),showLine:false,pointRadius:6,
      pointBackgroundColor:col,pointBorderColor:col,pointBorderWidth:1,
      datalabels:{display:true,anchor:i===maxI?'end':'start',align:i===maxI?'top':'bottom',offset:8,formatter:v=>v?.toLocaleString('zh-TW')??'',clip:false,color:'#000',font:{size:10}}});
  ...
  chart=new Chart(cvs,{
    ...
    options:{
      ...
      scales:{x:{type:'linear',min:-0.5,max:26.8,grid:{display:false},ticks:{display:false}},y:{ticks:{callback:v=>v.toLocaleString('zh-TW')}}}
    },
