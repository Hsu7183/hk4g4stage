...
function drawChart (ymArr, T, L, S, P) {
  ...
  const mkLast = (d, col) => ({
    ...
    showLine: false, pointRadius: 0   // 改成 0 → 最後數值用 lastValueLabel 另畫
  });
  ...
  /* --- 最後數值固定畫在右側 --- */
  const lastValueLabel = { id: 'lastValueLabel', afterDraw (c) {
    const { ctx, scales: { x, y }, chartArea: { right } } = c;
    ctx.save();
    ctx.font = '10px sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#000';

    // 依照 datasets[0~3] (T, L, S, P) 依序標注
    [T, L, S, P].forEach((d, idx) => {
      const lastY = y.getPixelForValue(d[d.length - 1]);
      const txt   = d[d.length - 1].toLocaleString('zh-TW');
      ctx.fillText(txt, right + 6, lastY);
    });
    ctx.restore();
  }};
  ...
  chart = new Chart(cvs, {
    ...
    options: {
      ...
      layout: { padding: { bottom: 42, right: 80 } },   // 右側多 80px
      plugins: {
    ...
    plugins: [stripe, mmLabel, lastValueLabel, ChartDataLabels]
  });
}
