...
function analyse(raw){
  ...
  const Q=[],tx=[],dates=[],tot=[],lon=[],sho=[],sli=[];
  ...
  rows.forEach(r=>{
    ...
    const gain=pts*MULT-fee-tax,gSlip=gain-SLIP*MULT;
    ...
    dates.push(ts); tot.push(Cum); lon.push(CumL); sho.push(CumS); sli.push(CumSlip);
  });

  if(!tx.length){alert('沒有成功配對的交易');return;}
  renderTable(tx); drawChart(dates,tot,lon,sho,sli);
}
...
function drawChart(tsArr,T,L,S,P){
  ...
  const X=tsArr.map(ts=>{
    const ym=ts.slice(0,6),day=+ts.slice(6,8),
          days=new Date(+ym.slice(0,4),+ym.slice(4,6),0).getDate();
    return idx[ym]+(day-0.5)/days;   // 0.5–(days-0.5)/days
  });
  ...
  const mkLine=(d,col,f=false)=>({data:d,stepped:true,borderColor:col,borderWidth:2,
    pointRadius:4,pointBackgroundColor:col,pointBorderColor:col,pointBorderWidth:1,fill:f});
  const mkLast=(d,col)=>({data:d.map((v,i)=>i===d.length-1?v:null),showLine:false,pointRadius:6,
    ...
    datalabels:{display:true,anchor:'center',align:'left',offset:10,clip:false,
  ...
  chart=new Chart(cvs,{
    ...
    options:{
      ...
      layout:{padding:{bottom:42,right:80}},
      plugins:{
      ...
      scales:{
        x:{type:'linear',min:-0.2,max:27.5,grid:{display:false},ticks:{display:false}},
