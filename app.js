...
function analyse(raw) {
  ...
  rows.forEach(r => {
    ...
    tr.push({ pos, tsOut: ts, priceOut: price, actOut: act,
...
function drawChart(dateArr, T, L, S, P) {
  ...
  const X = dateArr.map(ts => {
    ...
    const offset = (d - 1) / (days - 1) - 0.5;          // -0.5(1號) → +0.5(月末)
    return monthIdx[ym] + offset;
  });
  ...
  const stripe = {
    id: 'stripe', beforeDraw(c) {
      ...
      ctx.save();
      months.forEach((_, i) => { ctx.fillStyle = i % 2 ? 'rgba(0,0,0,.05)' : 'transparent';
  ...
  const mkLast = (d, col) => ({
    ...
    datalabels: {
      ...
      formatter: v => v?.toLocaleString('zh-TW') ?? '', clip: false, color: '#000', font: { size: 10 }
    }
  });
  const mkMark = (d, i, col) => ({
    ...
    datalabels: {
      display: true,
      anchor: i === maxI ? 'end' : 'start', align: i === maxI ? 'top' : 'bottom',
      offset: 8,
      formatter: v => v?.toLocaleString('zh-TW') ?? '', clip: false, color: '#000', font: { size: 10 }
    }
  });
  ...
  chart = new Chart(cvs, {
    ...
    options: {
      ...
      layout: { padding: { bottom: 42, right: 30 } },    // 右側多留 30px
      plugins: {
      ...
      scales: {
        x: { type: 'linear', min: -0.5, max: 26.5,     // 26.5 ⇒ 為右側文字預留空間
