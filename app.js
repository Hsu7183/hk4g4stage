...
function analyse(raw) {
  ...
  rows.forEach(r => {
    ...
    if (ENTRY.includes(a)) {
      Q.push({ side: a === '新買' ? 'L' : 'S', tsIn: ts, pIn: price, tIn: a });
      return;
    }
    const i = Q.findIndex(o => (o.side === 'L' && EXIT_L.includes(a)) ||
                               (o.side === 'S' && EXIT_S.includes(a)));
    if (i === -1) return;
...
function renderTable(list) {
  ...
  list.forEach((t, i) => body.insertAdjacentHTML('beforeend', `
    <tr><td rowspan="2">${i + 1}</td>
        <td>${fmtTs(t.pos.tsIn)}</td><td>${t.pos.pIn}</td><td>${t.pos.tIn}</td>
        <td>—</td><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>
    <tr><td>${fmtTs(t.tsOut)}</td><td>${t.priceOut}</td><td>${t.actOut}</td>
        <td>${fmt(t.pts)}</td><td>${fmt(t.fee)}</td><td>${fmt(t.tax)}</td>
        <td>${fmt(t.g)}</td><td>${fmt(t.cT)}</td>
        <td>${fmt(t.gSlip)}</td><td>${fmt(t.cP)}</td></tr>`));
  tbl.hidden = false;
}
...
function drawChart(tsArr, T, L, S, P) {
  ...
  const mkLast = (d, col) => {
    const x = X[X.length - 1] + 0.25;  /* 只往右 1/4 格 */
    ...
    return {
      ...
      datalabels: {
        ...
        offset: 4,
        clip: false,
        formatter: () => y.toLocaleString('zh-TW'),
        color: '#000',
        font: { size: 10 }
      }
    };
  ...
  chart = new Chart(cvs, {
    ...
    options: {
      ...
      layout: { padding: { bottom: 42, right: 60 } },  /* 右側留半格空間 */
      plugins: {
