/* v2025-06-15 +lastLabelFix (每筆顯示；最後數值永遠在點右側) */
...
function drawChart(dateArr,T,L,S,P){
  ...
  const X=dateArr.map((_,i)=>i);                              // 每筆一點
  ...
  const mmLabel={id:'mmLabel',afterDraw(c){
    const {ctx,chartArea:{left,bottom,right}}=c,
          w=(right-left)/(X.length-1||1);
    ctx.save();
  ...
  const mkLast=(d,col)=>({
    ...
    datalabels:{
      ...
      anchor:'center',align:'left',offset:10,          /* ← 右邊 10px */
